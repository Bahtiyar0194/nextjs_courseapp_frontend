...
export default function CreateTest() {
    ...
    const createTestSubmit = async (lesson_id) => {
        ...
        if (blocks.length > 0) {
            for (let index = 0; index < blocks.length; index++) {
                ...
                for (let input of answer_inputs) {
                    ...
                    let property_select = block.querySelector('[data-answer-select="' + input.getAttribute('data-answer-input') + '"]');

                    answers.push({
                        answer_value: input.value,
                        answer_property: property_select.value
                    });
                    ...
                    if (property_select.value === '') {
                        not_selected_properties.push(property_select);
                    }

                    if (not_selected_properties.length > 0) {
                        for (let select of not_selected_properties) {
                ...
                if (question_input.value == '' || empty_answers > 0 || not_selected_properties.length > 0 || properties.length === 0) {
        ...
        form_data.append('task_type_id', 4);
    ...
    return (
        <DashboardLayout showLoader={showFullLoader} title={intl.formatMessage({ id: "task.add_analytical_test" })}>
            <RoleProvider roles={[2]} redirect={true}>
                <Breadcrumb>
                    ...
                    {intl.formatMessage({ id: "task.add_analytical_test" })}
                </Breadcrumb>
                ...
                <div className="col-span-12">
                    ...
                    <div className="mt-4 relative">
                        <AddTag items={properties} setItems={setProperties} className={"inactive p-4 mt-4"} tagClass={"tag-light"} tagInputId={"add-property-input"} label={"task.analytical_test.add_properties_for_this_test"} message={intl.formatMessage({ id: "a_property_with_this_value_already_exists" })} modal_title={intl.formatMessage({ id: "task.add_a_property" })} with_description={true} />
                    </div>
